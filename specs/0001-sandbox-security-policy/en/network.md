...
5. **Rate and reachability.** Bandwidth is a `resource.quota` dimension while reachability is here ([overview.md](./overview.md) §11.9). Should a `NetworkRule` carry its own rate ceiling, or does that recreate the dialect problem the unified object exists to prevent?
