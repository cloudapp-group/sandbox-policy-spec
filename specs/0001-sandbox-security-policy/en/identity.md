...
This spec defines the identity (`identity`) sub-policy of the `SandboxPolicy` object: **what names the acting parties are known by, which credentials a sandbox may use, and in what form they reach it**. It governs four things:

- **Identity naming** — the names under which Agents act and tools are reached in the platform (§2).
...
A sandbox runs an Agent, and that Agent calls tools. These two ends are the whole of identity, and they share one naming scheme:

| Object | Name |
| --- | --- |
| Agent | `agent://<tenant_type>/<cluster_id>/<cluster_namespace>/<agent_name>` |
| Tool | `tool://<tenant_type>/<cluster_id>/<cluster_namespace>/<tool_name>` |

`tenant_type` is a closed enum, not free text:

| Value | Meaning |
| --- | --- |
| `system` | Objects the platform itself owns and operates |
| `user` | Objects a tenant owns |

1. Five segments together identify one object: `<scheme, tenant_type, cluster_id, cluster_namespace, name>`. Two names MUST be compared segment by segment; the same name under a different `cluster_namespace` is a different object, with no implied inheritance or visibility.
2. Every segment MUST be non-empty and MUST NOT contain `/`. That is the whole of this spec's character-set constraint — anything stricter belongs to the platform's own object registry.
3. A name is not a credential, and not a secret. Knowing a `tool://` URI — or writing one into a policy — grants nothing; a URI may appear in audit and error payloads outside the redaction rule of §9, because it names an object and does not carry the key that opens it.
4. A URI names an object, not a run. The same Agent running in two sandboxes is two identity instances (§6.3) sharing one name — instance distinction happens at issuance, not at naming.
5. The naming is load-bearing across this module: `audience` (§6.2) uses it to name what an identity is minted for; audit (§10) uses it to attribute an action to its identity; and the `identity` reachability mode of [network.md](./network.md) resolves objects by it rather than by address.
...
policy:
  identity:
    ...
    workloadIdentity:
      ...
      audience:       [string]                   # required when enabled; §2 URIs
...
| --- | --- | --- |
| `proxy` | Nothing | Yes, for the declared destinations and scope (§5) |
...
2. Under `file` and `env`, the value is inside the sandbox and this module's guarantees end there. A credential the workload can read is a credential the workload can send anywhere `network` permits, log to stdout, or write into a file that a snapshot later carries out ([overview.md](./overview.md) §8.3.2). These modes exist because real workloads use SDKs that read `~/.aws/credentials` or `AWS_SECRET_ACCESS_KEY`, and a spec that only offered `proxy` would be ignored rather than adopted. They are not equivalent to `proxy`, and §7 makes the platform say so.
...
4. **Exposure is narrow-only in merge** (§11): the ordering is `none` > `proxy` > `file` > `env`. A lower-precedence source that chose `proxy` MUST NOT be widened to `file` or `env` by a higher-precedence one.

### 4.1 What `proxy` requires of the deployment, stated plainly
...
## 5. Destination binding and scope
...
4. `destinations` does not widen `network`. A destination permitted here but denied by `network` stays unreachable; the modules intersect. The converse also holds and is worth stating: `network.internal.mode: allow` makes the cloud metadata endpoint reachable, and a credential obtained from it is one this module never issued and cannot revoke ([network.md](./network.md) §2.2.3). A policy pairing that setting with a `proxy` binding therefore carries the `metadata_endpoint_reachable` warning, because the guarantee in §4.1 holds only for the credentials this module controls. A binding whose destinations are entirely unreachable under the effective network policy MUST be reported as a `policyWarnings` entry `{field: "policy.identity.secrets", name, reason: "destinations_unreachable"}` — an author who believes a credential is in use when nothing can reach its destination should learn it at create time.
5. Where a credential is short-lived and the platform mints it (§6), the minted credential's own audience MUST be constrained to the binding's destinations where the issuing system supports it. Scope enforced at the token is stronger than scope enforced at the proxy, because it survives the proxy being wrong.

## 6. Workload identity and lifetime

1. When `workloadIdentity.enabled` is true, the platform MUST be able to present the Agent the sandbox carries to external services as a distinct, verifiable identity, named in the `agent://` form of §2 and scoped to `audience`, whose every entry is a §2 URI naming what the identity is minted for. An identity with no audience is a bearer token with extra steps.
2. `audience` is REQUIRED when `enabled` is true. Rejecting an empty audience with `400 INVALID_POLICY` is deliberate: the failure mode of a missing audience is a credential accepted by a service nobody meant to authorize. An entry that is not in the §2 form is rejected the same way: an audience that resolves to no object constrains nothing.
3. The identity MUST be bound to the sandbox instance, not to the template, the profile, or the caller. Two sandboxes from one template are two identity instances, so that revoking one does not revoke the other and an audit trail can attribute an action to an identity and the sandbox that carried it.
...
7. Under `exposure: file` or `env`, rules 5 and 6 are **best-effort by construction** and the platform MUST say so: a value already read into the workload's memory cannot be recalled. This is the concrete cost of those modes, and it belongs in the capability report (§7), not in a footnote.
...
| `workloadIdentity.enabled` | `bool?` | — | `true` under `managed` | §6.1. |
| `workloadIdentity.audience` | `[string]?` | MUST be non-empty when `enabled` is true; entries in the §2 form. | `[]` | §6.2. |
| `workloadIdentity.ttlSec` | `int?` | > 0, ≤ deployment maximum. | `900` | §6.4. |
| `defaultExposure` | `enum?` | `proxy` \| `file` \| `env` \| `none` | `proxy` | Default `exposure` for bindings that omit it. |
| `secrets` | `[SecretBinding]?` | Per §3.1. `name` unique. | `[]` | The declared credentials. |
| `onViolation` | `enum?` | `deny` \| `kill` | `deny` | §8. |
| `audit` | `enum?` | `none` \| `metadata` | `none` | Audit level for **ordinary** credential use (§10). It does not suppress violation events ([overview.md](./overview.md) §8.1.4). |
...
`baseline` sets `defaultExposure: file` rather than `proxy` for a reason worth stating: `proxy` requires an egress enforcement point (§4.1) that not every deployment has, and a tier that resolved to an `unsupported` field would make `tier: baseline` fail under `enforcement: strict` on those deployments. `file` at least makes the exposure explicit and auditable. `restricted` selects `proxy`, and a deployment that cannot enforce it finds out at create time — which is the correct moment.

### 7.2 Shadow evaluation support
...
| Action | Result |
| --- | --- |
| `deny` (default) | The request is forwarded without the credential (§5.2), or the operation fails with the module's error (§9). The process keeps running. |
...
3. Either action emits a violation event, at every audit level (§10).

## 9. Errors

| Code | HTTP | Payload | When |
| --- | --- | --- | --- |
| `INVALID_POLICY` | 400 | `{field, reason}` | Empty `audience` under `enabled: true`, or an entry not in the §2 form; empty `destinations` under `proxy`; a field the mode does not use (§3.1.3); `ttlSec` above the deployment maximum. |
...
| `POLICY_IDENTITY_EXPOSURE_DENIED` | OS-level failure or `403`; audit event | `{name, exposure, action}` | The workload attempted to read a credential whose exposure mode hides it (§4.1). |
| `POLICY_IDENTITY_DESTINATION_DENIED` | audit event only | `{name, destination}` | A credential was withheld because the destination did not match (§5.2). The request itself proceeds unauthenticated, so this is not an error to the caller. |
| `POLICY_UNSUPPORTED` | 400 | `{field, state, capabilityVersion}` | Under `enforcement: strict`, the policy uses an exposure mode this deployment declares `unsupported` — most often `proxy` (§4.1). |
| `POLICY_GRANT_INVALID` | 400 | `{field, reason}` | A grant targets a non-grantable field (§11.1). |
...
2. `audit: metadata` adds the record of **ordinary** credential use: which binding was attached to which destination, when, and under which identity (§2). Not the credential.
3. Every credential the platform mints or attaches MUST be attributable to a sandbox, an identity (§2), a binding `name`, and an `effectivePolicyVersion`. "Which sandbox used this credential at 14:03?" MUST be answerable from the audit stream, because that is the question an incident starts with.
4. A binding resolved to `exposure: file` or `env` MUST emit an audit event at sandbox creation recording the residual exposure (§6.7). Handing a readable credential to partially trusted code is an event, not a silent field — the same treatment [filesystem.md](./filesystem.md) §9.4 gives `baselineExceptions`.
...
| `defaultExposure` | Most restrictive wins: `none` > `proxy` > `file` > `env` (§4.4). |
...
3. **Header spoofing does not attach.** A connection to an address outside `destinations` carrying a `Host` header that names an allowed destination does not receive the credential (§5.3).
...
6. **Audience is mandatory and named.** `workloadIdentity.enabled: true` with an empty `audience`, or with an entry not in the §2 form, is rejected with `400 INVALID_POLICY`.
7. **Identity is per instance.** Two sandboxes from one template present two identity instances (§6.3), and revoking one leaves the other working.
8. **Namespaces do not inherit.** A tool with the same name under two `cluster_namespace`s is two objects: reachability or `audience` declared for one opens nothing for the other.
9. **Renewal needs no cooperation.** A sandbox that makes authenticated requests across a period longer than `ttlSec` continues to succeed without performing any refresh itself (§6.5).
10. **Revocation is live.** Removing a binding stops subsequent requests from being authenticated, without restarting the sandbox (§6.6).
11. **Residual exposure is honest.** Under `exposure: env`, revocation does not retract a value the workload already read, the effective policy reports that residual exposure, and creation emitted the §10.4 event.
12. **No leakage in payloads.** No error, warning, or audit event produced by this module contains a credential value or a `secretRef`, including in the unresolvable-secret and exposure-denied cases (§9).
13. **Merge is narrow-only.** A template binding with `exposure: proxy` cannot be changed to `env` by a request of the same `name`; `destinations` intersect; `ttlSec` takes the minimum; `audience` intersects. A request that attempts to widen any of them is handled per [overview.md](./overview.md) §5 — rejected or reported, never silently applied.
14. **Grants.** A grant adding a `proxy` binding expires without action by the sandbox, after which requests are no longer authenticated. A grant attempting to change `exposure`, raise `ttlSec`, or set `mode: unrestricted` is rejected.
15. **Unsupported proxy fails closed.** On a deployment declaring `exposure: proxy` `unsupported`, a policy requesting it is rejected with `400 POLICY_UNSUPPORTED` under `enforcement: strict`, and under `bestEffort` is accepted with the binding reported inert — in neither case is it silently downgraded to `file` or `env`.
16. **Tier defaults.** `tier: restricted` with no identity fields resolves to `mode: managed`, `defaultExposure: proxy`, `workloadIdentity.enabled: true`, and no secrets — a sandbox that authenticates as nothing and holds nothing. `tier: compatibility` resolves to `mode: unrestricted` and changes nothing about today's behaviour.
17. **Shadow evaluation.** With `tier: baseline` and `auditTier: restricted`, a binding in `file` mode continues to work and emits a `shadow: true` finding naming the binding and `proxy`. No credential is minted for the shadow evaluation, and nothing observable inside the sandbox differs.
18. **Lifecycle.** A sandbox restored from a snapshot does not inherit secret bindings ([overview.md](./overview.md) §8.3); authenticated requests fail until the bindings are re-established.

## 13. Open questions

1. **Credential type taxonomy.** This spec treats every credential as an opaque value attached to outbound requests. Real credentials differ: a bearer token is attached to a header, an AWS signature is computed over the request, a client certificate is used in the handshake, and a database password is sent inside a protocol this module cannot parse. Does `SecretBinding` need a `type` with per-type attachment semantics, or is `proxy` mode honestly limited to HTTP-shaped credentials — in which case that limit MUST be stated in §4 rather than implied?
2. **Non-HTTP destinations.** §5 is written in terms of destinations, methods, and request paths, which presumes HTTP. A `proxy`-mode credential for a Postgres connection has no method or path, and the proxy would have to speak the wire protocol. Is `proxy` restricted to HTTP/HTTPS in v1, with non-HTTP credentials necessarily `file` or `env`?
...
7. **Naming authority and lifecycle.** Who may register an `agent://` or `tool://` name in a namespace, and how soon may a name be reused after its object is deleted? Recycling a name makes old audit trails point at an unrelated successor — answering "what did this identity do at 14:03" first requires establishing that it is still the same one.
...
- **Why the sandbox is not in the naming.** A sandbox is an execution environment, not an object that acts or is reached by a URI — the Agent it carries acts, the tools are what is reached. The platform already identifies instances by sandbox ID (the audit events of §10); giving the sandbox an identity URI would answer the same question with a second identifier and make "environment" and "principal" look alike. §2 therefore names two kinds of object, and §6.3 binds an identity to a sandbox instance — name and instance, each doing its own job.
- What this module *does* depend on is the presence of an egress enforcement point, which is the same dependency `network` L7 rules, `resource.limits.tokens`, and `resource.rate.network` have. A deployment that has none of the four has a coherent, weaker posture; a deployment that has it for one and not the others is probably misconfigured rather than deliberately limited.
- **On preferring `proxy` without mandating it.** An earlier draft of this module made `proxy` the only mode. It was rejected for the reason §4.2 gives: the SDK ecosystem reads credentials from files and environment variables, and a policy language that cannot express what deployments actually do gets bypassed rather than adopted. The compromise is that `file` and `env` remain available, remain audited at creation, and are described as what they are — the residual-exposure language in §6.7 and §10.4 is deliberately blunt.
