...
Not in scope: inter-sandbox and sandbox-to-host process isolation, which is a property of the substrate rather than a policy field ([overview.md](./overview.md) §12.3) — there is no field for it because there is nothing for a user to decide. Also not in scope: detecting anomalous, zombie, or malicious processes ([overview.md](./overview.md) §2.3.1), and process-count or CPU consumption, which are `resource` dimensions.
...
### 4.3 The baseline sets `syscall/1` and `syscall/1-vm`
...
**The VM substrate has its own baseline set, `syscall/1-vm`.** This is the only substrate-specific set in the proposal, and it exists because of where enforcement sits rather than because of what the kernel can do.

On the container substrate the filter is installed by the host kernel, so a workload holding root inside the sandbox is not root over its own restrictions. On the VM substrate the filter is installed by a supervisor process *inside the guest*, sharing a kernel with the workload ([overview.md](./overview.md) §12.2). A workload that can attach to that supervisor, read its memory, or write to it can subvert every restriction installed after that moment — so the syscalls that reach into another process's address space are not a debugging convenience there, they are the bypass.

`syscall/1-vm` is `syscall/1` plus:

```yaml
deniedSyscalls:
  - ptrace              # attach to, inspect, and modify another process
  - process_vm_readv    # read another process's memory without attaching
  - process_vm_writev   # write another process's memory without attaching
```
...
1. Baseline sets are **versioned and immutable**. `syscall/1` is the first set above and `syscall/1-vm` the second; a published version of either MUST NOT be modified in place.
...
2. **Restricted-tier syscall set.** `tier: restricted` currently expands to the same baseline set as `tier: baseline` ([overview.md](./overview.md) §7.1). Should `restricted` pull in a second, larger versioned set (adding `mount`, `umount2`, `unshare`) rather than only tightening privilege and persistence? One objection to this has now been answered from an unexpected direction: `syscall/1-vm` (§4.3) establishes that more than one published set can coexist and be selected by something other than the policy author, so "a second set is a second thing to maintain" is a cost the proposal has already accepted once. What remains is the original question of whether a tier may select a set at all, given that §7.1 rule 1 confines a tier to default values of module fields — `baselineVersion` is such a field, so the answer is probably yes, and the real question is which entries.
...
- Every requirement here is expressible with per-process, unprivileged mechanisms (a seccomp-style filter installed at process creation, the kernel's no-new-privileges flag, a bounding capability set, per-session process-group tracking). As elsewhere in this proposal, the spec fixes *properties* and leaves mechanism selection open. Shadow evaluation (§6.1) is no exception: filter mechanisms in common use provide a log-and-permit action, which is what rule 1 there requires.
- **This module's mechanisms are substrate-independent; the assurance they provide is not.** Every mechanism above is per-*process* rather than per-kernel: `securityContext` on the container substrate and in-guest configuration on the VM substrate reach the same properties, and narrowing one sandbox's syscall surface cannot destabilise a neighbour on a shared kernel, because the filter is attached to processes rather than to the kernel. An earlier draft stopped there, and stopping there was wrong. **Where the filter is installed from differs, and that decides what the field guarantees.** On the container substrate the host kernel installs it, so root inside the sandbox is not root over the restriction. On the VM substrate a supervisor inside the guest installs it, sharing a kernel with the code it constrains — so the guarantee holds only while the workload cannot reach that supervisor, which is why §4.3 gives the VM substrate a baseline set that denies the syscalls for reaching into another process. A deployment on either substrate may declare §3 and §4 `enforced`; on the VM substrate it may do so **without qualification** only when both conditions of [overview.md](./overview.md) §12.2 hold.
- The two fields with the most assembly required on the container substrate are `allowDaemonize` and `runAsNonRoot`, neither for a deep reason. `allowDaemonize: false` needs session tracking plus denial of the detach syscalls, which is a filter entry combined with process-group supervision rather than a single switch. `runAsNonRoot`'s create-time check (§3.5.2) has a direct `securityContext` equivalent, but its runtime half — denying `setuid(0)` even to a capability holder — is a filter entry that has to be installed alongside it, or the field only holds at start.
