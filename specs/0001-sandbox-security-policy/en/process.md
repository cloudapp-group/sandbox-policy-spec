...
- **This module is substrate-independent, and that is worth stating because an earlier draft implied otherwise.** Every mechanism above is per-*process*, not per-kernel: `securityContext` on the container substrate and in-guest configuration on the VM substrate reach the same properties, and a deployment on either is expected to declare §3 and §4 `enforced` ([overview.md](./overview.md) §12.2). Narrowing one sandbox's syscall surface cannot destabilise a neighbour on a shared kernel, because the filter is attached to processes rather than to the kernel. What a dedicated kernel buys is latitude in host-wide settings, not the ability to filter — and conflating the two is how a spec acquires a substrate requirement it does not need.
