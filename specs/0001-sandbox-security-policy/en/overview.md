...
| **Resource** — request/disk rate ceilings, windowed token budgets (minute–month + lifetime), and token accounting | [resource.md](./resource.md) |
...
| Filesystem | Nothing user-facing. Which host paths may be mounted is an admission decision the deployment already makes outside this object ([filesystem.md](./filesystem.md) §1). | No protection for sensitive paths *inside* the sandbox (`~/.ssh`, `~/.aws/credentials`, `/etc/shadow`); no path-level read-only/deny policy of any kind. |
...
| Resource | Steady-state CPU/memory quotas, set by ordinary orchestrator configuration; idle timeout with kill/pause. | No rate ceiling on outbound requests or disk I/O; no windowed token budgets (minute–month) or lifetime budgets; no token metering; no exceed actions, notifications, or human-approval flow. |
...
| **Filesystem** | In-sandbox credential theft; abuse of whatever the sandbox can reach on disk, mounted or not | Every filesystem access by **every** process | Secrets already in process memory or passed as env vars; which host paths were mounted in the first place |
| **Resource** | Token burn, noisy-neighbour effects | Substrate rate limiters + outbound HTTP metering, per sandbox | Actions that are harmful but cheap; runaway loops that spend no tokens ([resource.md](./resource.md) §1) |
...
| List fields (`denyPaths`, `allowedSyscalls`, ...) | Higher-precedence entries are appended to lower-precedence entries, then deduplicated. |
| Rule lists (`network.egress.rules`, `network.ingress.rules`) | Rules are appended across sources and evaluated in explicit `priority` order, not in source order ([network.md](./network.md) §4.5). Priority orders rules; it never confers authority (§4.5.4 there). |
...
| --- | --- |
| **Operator** | The party running the platform. Sets deployment-wide bounding constraints such as the maximum grant TTL, and the admission decisions that sit outside this object entirely — which host paths may be mounted among them ([filesystem.md](./filesystem.md) §1). |
...
| --- | --- | --- |
| Network | Deny-all in both directions, and the private network unreachable: only named rules pass. | `egress.defaultAction: allow`, `ingress.defaultAction: allow`, `internal.mode`, or `tier: baseline`. |
...
| --- | --- | --- | --- |
| Network | `egress.defaultAction: allow`, `ingress.defaultAction: allow`, `internal.mode: deny` | `egress.defaultAction: allow`, `ingress.defaultAction: deny`, `internal.mode: deny` | `egress.defaultAction: deny`, `ingress.defaultAction: deny`, `internal.mode: deny` |
| Filesystem | `mode: baseline`, `baseline/1` | `mode: baseline`, `baseline/1` | `mode: baseline`, `baseline/1`, `audit: metadata` |
...
| Resource | no ceilings, no windowed limits | no ceilings, no windowed limits | no ceilings, `onExceeded: hold` |

`tier: unrestricted` expands to each module's documented opt-out — `network.egress.defaultAction: allow` and `ingress.defaultAction: allow` with no added denies, `filesystem.mode: unrestricted`, `exec.mode: unrestricted`, `process.mode: unrestricted`, `identity.mode: unrestricted`. It does **not** relax `network.internal.mode`, which stays `deny` under every tier ([network.md](./network.md) §6): reaching the private network is a statement about a deployment's topology, which no tier can make on its behalf. It is the tier for trusted, human-authored workloads, and every use of it is visible in the effective policy and its snapshot.
...
| --- | --- | --- | --- |
| **network** | Connection failure (TCP reset / drop), or `403` for a rejected L7 request | The **sandbox** | L4 enforcement sees packets, not process identity. Attributing a connection to a process is unreliable at that layer, and a best-effort attribution would kill the wrong process — worse than not killing at all. Where the enforcement point is a shared gateway the termination is additionally asynchronous ([network.md](./network.md) §4.8.3). |
...
| **1** | `SandboxPolicy` API model; legacy-field normalization; `policy.network` end-to-end — both directions, priority ordering, L4 rules, and `internal.mode`; conflict detection; effective-policy versioning and snapshots (§4.1); the violation response model and its always-on violation events (§8.1); the capability set, `GET /capabilities`, `policy.enforcement`, and inert-field reporting (§8.2); concurrency control and `status` (§4.2, §4.3); the principal and authority matrix (§5.2); the machine-readable schema and conformance suite (§11.16); SDK `policy=`. |
| **2** | Resource: rate ceilings with shaping counters, windowed token budgets (`minute`–`month` + `lifetime`), `onExceeded` actions (`warn`/`pause`/`hold`/`kill`), notifications and webhook, hold approval API, usage exposure. |
| **3** | Filesystem: baseline sensitive-path protection, `readOnlyPaths` / `denyPaths` / `writableRoots`. |
...
9. **Reachability vs consumption.** Egress port/protocol rules live in `network`, the request-rate ceiling lives in `resource.rate`. Is that seam (what may be reached vs how fast it may be used) the right one, or should a user be able to state "443 to this CIDR, at most 10 requests/s" in one place?
...
18. **Ingress authentication and image are under-specified relative to their blast radius.** **Ingress:** the structural half is now closed — [network.md](./network.md) §2.1 gives ingress the same rule shape as egress, with port, protocol, source peer, priority, and L7 matchers. What is still missing is *authentication*: token binding, expiry, revocation, and the difference between "reachable" and "reachable by an authenticated caller". A public URL is a capability grant, not a network attribute, and none of the fields that would make it one exist yet. **Image:** every rule in `process`, `filesystem`, and `identity` executes inside an image this object says nothing about; a mutable tag or an unsigned registry undoes runtime enforcement without violating a single policy field. Minimum semantics for each, if they become modules: for ingress, protocol/port, source constraint, auth mode, identity binding, expiry, revocation, and a `public` flag that is never the default; for image, an immutable digest, allowed registries, signature or attestation status, and the startup UID the `process` rules will apply to.
...
| `resource.rate.disk.*` | Byte-rate and operation-rate limits in the substrate's I/O path |
| `network` connection state (§4.7) | Connection tracking in the data path |
| `network` L4 rules for addresses and CIDRs | Packet filtering at L3/L4, in priority order |
| `network` L7 rules | An HTTP/HTTPS proxy in the path, terminating TLS for `https` |
