...
- `netBandwidthKbps` shaping and `netEgressBytes` metering both attach to the sandbox's network interface, and on the container substrate that interface belongs to the shared network namespace ([network.md](./network.md) §4.9). Unlike network *policy*, this does not create a scope conflict — bandwidth and byte counts are additive, so per-container cgroup accounting still attributes them correctly. It is worth noting only because the two modules read the same interface and reach opposite conclusions about whether sharing it is a problem.
