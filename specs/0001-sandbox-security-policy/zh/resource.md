...
`onExceeded` 是本模块唯一的响应字段：依 [overview.md](./overview.md) §8.1.6，**这里没有 `onViolation`**。两者不是恰好落在不同模块里的两个替代品 —— 它们回答的是不同的问题，而 §8.1.1 已经把哪个是哪个钉住了。超限意味着工作负载待在被给定的每一条边界之内、把预算花完了；违规意味着它越过了其中一条。这就是为什么本动作集有 `warn` 与 `hold` 而 `onViolation` 两者都没有：关于"需要更多预算"有东西可供人来决定，而一条关于超支的警告并不让任何保护处于关闭状态，因为预算从来就不是一道防意图的保护。

两套动作集唯一共有的是 `kill`，而即便是它，作用范围也不同。在这里它终止**沙箱**，因为消耗量是一个沙箱级的量，不存在单一的罪魁进程 —— 与 `network` 被迫采用的是同一个粒度，只是理由不同（[overview.md](./overview.md) §8.1.3）。
...
依 [overview.md](./overview.md) §7.2.5，本模块声明自己的立场：**它没有影子评估，因为它没有可影子的东西。**

理由就是上面那一段。影子评估是把一个被强制执行的分级与一个更严的分级作比较，而没有任何分级会改动这里的任何字段。不存在一套更严的限额供 `auditTier: restricted` 去比对，所以指向本模块的 `auditTier` 解析为零条发现 —— 不是因为机制缺失，而是因为比较本身是空的。

这不是一处日后要靠"补上影子支持"来填的缺口。如果"一个更紧的预算本来会拦掉什么"这个问题真的值得回答，那么答案已经存在，而且更好：`onExceeded: warn`（§6）就是那个功能，只是从另一个方向抵达。想观察一个候选限额的部署就把这个限额连同 `warn` 一起设上，拿到的是带真实计数器的超限事件，而不是对一个还没人选定的数字做并行求值。一个结果配两套机制，正是 §10.1 已经为授权拒绝过的东西，而其推理在此原样成立。

有一处区分值得保持清楚：本提案其他地方的一条影子发现意味着"这个操作本来会被拒绝"。而在这里，对应的问题是"这个预算本来会被耗尽"，那关乎一个计数器抵达某个数值，而不是对某个操作作出判定 —— 而计数器正是本模块真实暴露的东西（§9），而不是影子出来的东西。
...
- **基质映射。** `quota` 与每一个窗口计数器都建立在 cgroup 计量之上，而那在两种基质上是同一个接口（[overview.md](./overview.md) §12.2），所以 `cpuMillicores`、`memoryMiB`、`cpuSeconds` 与各超限动作在任一边都直接映射。有两个维度在部署把它们声明为 `enforced` 之前值得再看一眼：

  | 维度 | 需要核对什么 |
  | --- | --- |
  | `diskWriteBytes` | 要求是写入沙箱*可写*设备的字节数（§5.1）。在 VM 基质上那是客户机的块层。在容器基质上，落在联合文件系统上层的写入可能不像独立卷那样出现在块设备统计里，因此一个无法完整看到它们的部署声明 `partial`，并文档化哪些写入被计入 —— 依 [overview.md](./overview.md) §8.2.1 规则 3，这个收窄必须被描述出来，因为"你的一部分写入被计入了"在不知道是哪一部分时是无法使用的。 |
  | `llmTokens.*` | 它们来自出站 HTTP 路径、而不是内核，因此需要 `network.rules` 所需的那同一个 L7 代理。出站路径上没有代理的部署根本无法计量 Token，于是把它们声明为 `unsupported`。这是本提案中唯一一处跨模块的实现依赖：两个模块，一套机制。 |
- `netBandwidthKbps` 的整形与 `netEgressBytes` 的计量都附着在沙箱的网络接口上。在容器基质上沙箱是一个 Pod（[network.md](./network.md) §4.9），所以那个接口属于该沙箱、不属于别的任何东西 —— 与 VM 基质是同一种安排。因此本模块不需要自己的作用域附注：接口、计数器所依附的 Pod cgroup，以及策略的主体，全都是同一个单位。请注意，一个沙箱*内部*的多个容器共享这两者，而那是正确的、不是问题，因为消耗预算是沙箱的属性，而不是构成它的那些容器的属性。
