...
- `netBandwidthKbps` 的整形与 `netEgressBytes` 的计量都附着在沙箱的网络接口上，而在容器基质上那个接口属于共享的网络命名空间（[network.md](./network.md) §4.9）。与网络*策略*不同，这不产生作用域冲突 —— 带宽与字节数是可加的，所以按容器的 cgroup 计量仍然能正确归因。它值得一提，仅仅是因为这两个模块读的是同一个接口，却对"共享它是否构成问题"给出了相反的结论。
