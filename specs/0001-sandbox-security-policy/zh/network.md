...
本提案中其他每一个模块的强制执行都落在沙箱单位上。本模块并不总是如此，而这个差异是基质的属性，不是一种选择：

1. 本策略在沙箱所处的**网络命名空间**上被强制执行。在 VM 基质上，那个命名空间恰好属于唯一一个沙箱，作用域与沙箱重合。
2. 在容器基质上，一个沙箱是一个容器（[overview.md](./overview.md) §12.1），而同处一地的容器**共享**一个命名空间。因此强制执行作用域比沙箱更宽。
3. 当两个或更多沙箱共享一个命名空间时，它们的 `policy.network` **必须**解析为完全相同的生效配置。一个会把沙箱放进某个命名空间、而该命名空间既有生效网络策略与之不同的创建请求，**必须**以 `400 POLICY_NETWORK_SCOPE_CONFLICT` 拒绝，携带冲突的字段以及那个建立了当前配置的沙箱。
4. 平台**不得**通过取最严值、取并集或取最近一次来消解这种冲突。这三者中的每一个都会静默地让一个沙箱的策略去治理另一个沙箱的流量，而那既是本对象并未描述的一种边界，也是它所能产生的最难调试的失败：一位运维读任一沙箱的生效策略，看到的都是一份与实际数据包不符的文档。
5. 规则 3 是针对*解析后*配置的创建时检查，而不是文本比较。两个通过不同来源抵达同一份生效策略的沙箱满足它。

规则 3 禁止了大多数容器部署实际在用的那种「工作负载 + sidecar」形态，而本规格目前还没有对它的答案 —— 见 [overview.md](./overview.md) §11.14。把这个冲突写成一次拒绝是刻意的：它让这处缺口在创建时、在它真实存在的那个基质上变得可见，而不是让一个 mesh 代理悄悄继承一份本为它旁边那个工作负载准备的锁定，或者反过来。
...
| `POLICY_NETWORK_SCOPE_CONFLICT` | 400 | `{fields, establishedBy}` | 一个沙箱将加入某个共享网络命名空间，而该命名空间的生效策略与之不同（§4.9.3）。 |
