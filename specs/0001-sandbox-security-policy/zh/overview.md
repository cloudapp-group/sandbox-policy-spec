...
- 所有策略错误使用 `POLICY_` 前缀：`POLICY_NETWORK_*`、`POLICY_FS_*`、`POLICY_EXEC_*`、`POLICY_PROCESS_*`、`POLICY_RESOURCE_*`、`POLICY_GRANT_*`、`POLICY_UNSUPPORTED`。
...
| **1** | `SandboxPolicy` API 模型；遗留字段规范化；`policy.network` 端到端；冲突检测；生效策略的版本化与快照（§4.1）；违规响应模型及其常开的违规事件（§8.1）；能力集、`GET /capabilities`、`policy.enforcement` 与失效字段上报（§8.2）；SDK `policy=`。 |
...
14. **同处一地、网络需求不同的沙箱。** §12.1 把容器基质上的沙箱单位定为一个容器，而 [network.md](./network.md) §4.9 要求同处一地的沙箱解析出相同的网络策略，因为它们共享一个网络命名空间。这两条规则合在一起，禁止了容器部署实际最常见的那种形态：一个几乎什么都不该访问的工作负载容器，旁边是一个必须访问控制面的 mesh 或遥测 sidecar。有四条出路，而没有哪一条显然是对的。把声明为基础设施的容器从"策略相同"规则中豁免出去 —— 那要求平台去判定什么算基础设施。把命名空间的生效策略定为同处各策略的并集 —— 那会放宽工作负载的边界，与 §5 相抵。把 sidecar 移出 Pod —— 那并非总是可行。或者最终还是把沙箱单位定为 Pod，并接受那些按容器的字段需要一个展开步骤。这个问题会卡住任何使用 sidecar 的容器部署，因此它不能带过阶段 1。
...
| | **VM 基质** | **容器基质** |
| --- | --- | --- |
| 沙箱单位 | 一个 MicroVM，独占内核 | **一个容器** |
| 策略附着点 | VM 配置 + 客户机内强制执行 | 容器 `securityContext`、cgroup，以及 CNI/数据路径 |
| 沙箱间隔离 | VM 边界的属性 | 命名空间与该节点隔离姿态的属性 |

容器基质上的沙箱单位是**一个容器，而不是一个 Pod**。这个选择让策略对象与 `securityContext` 对齐 —— 后者本就是按容器的 —— 于是 `process`、`filesystem`、`resource` 的字段无需展开步骤就能映射：一个 init container 是它自己的沙箱、有自己的策略，一个 sidecar 是另一个。它有一处后果必须写明而不能留给人去发现，因为那是这套映射唯一对不齐的地方：

**一个 Pod 内的容器共享同一个网络命名空间。** 因此 `policy.network` 的强制执行作用域比沙箱单位**更宽**。同一 Pod 内的两个沙箱不可能拥有不同的网络策略，而一份假装可以的规范，描述的是一种并不存在的隔离。[network.md](./network.md) §4.9 把这一点规范化：同处一地的沙箱若其网络策略不能解析为完全相同，**必须**被拒绝，而不是静默取最严的那个 —— 因为把一个容器的策略静默施加到另一个容器上，是这个对象所能产生的最难调试的失败。这与 mesh sidecar 之间的张力是真实且未解决的（sidecar 合理地需要一个比它旁边的工作负载更宽的网络姿态）；它被记为 §11.14。

### 12.2 两种基质在哪里分岔

这个对象的大部分是与基质无关的。下面这些机制在两边都是通用件，而任一基质上的部署都被期望把它们声明为 `enforced`（§8.2）：
...
- VM 基质的每沙箱一个内核模型，让内核级机制可以按沙箱启用而不影响宿主，这也是它在 §12.1 中排在前面的原因。它不是本提案的前置条件。
