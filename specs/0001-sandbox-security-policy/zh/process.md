...
这个模块之所以存在，是因为提案其余部分已经明确指出了一处缺口。[exec.md](./exec.md) §1 与 §3.5 声明 `exec` 策略是一道控制接口门禁，**不是**围堵边界：一条命令一旦被放行，它启动的进程可以 `fork`/`exec` 任何东西而不再经过 `exec` 求值，而对 Agent 生成的代码来说，被放行的那条命令典型就是一个解释器。[overview.md](./overview.md) §2.3 给出了推论 —— 对自启进程的围堵，必须来自在控制接口之下强制执行的模块。
...
不在范围内：沙箱之间以及沙箱与宿主之间的进程隔离 —— 那是基质的属性，而不是一个策略字段（[overview.md](./overview.md) §12.3）—— 之所以没有对应字段，是因为没有任何东西需要用户来决定。同样不在范围内：识别异常、僵尸或恶意进程（[overview.md](./overview.md) §2.3.1），以及进程数与 CPU 消耗 —— 那些是 `resource` 的维度。
...
在 `baseline` 分级下默认为 `false`，在 `restricted` 下为 `true`（[overview.md](./overview.md) §7.1）。这个默认值刻意是宽松的：`sudo` 是一个 setuid 二进制，[exec.md](./exec.md) §3.4 把它列在平台预期要解析的包装器之中，而使用它的镜像很常见。默认拒绝提权会让这些镜像在离肇因策略很远的地方崩掉。[filesystem.md](./filesystem.md) §6.1 的凭据路径先例在性质上不同 —— 读 `~/.aws/credentials` 在正当工作负载里很少见，而 `sudo` 不是。
...
### 4.3 基线集合 `syscall/1` 与 `syscall/1-vm`
...
| 被排除 | 原因 |
| --- | --- |
| `ptrace` | 调试器要用它，而 [exec.md](./exec.md) §3.4 把 `strace` 与 `ltrace` 列在平台会解析的包装器之中。在基线里拒绝它，会与提案已经预期能工作的一个面自相矛盾。**在 VM 基质上这条排除不成立** —— 见下面那个与基质相关的集合。 |
...
**VM 基质拥有自己的基线集合 `syscall/1-vm`。** 这是本提案中唯一一个与基质相关的集合，它之所以存在，是因为强制执行位于何处，而不是因为内核能做什么。

在容器基质上，过滤器由宿主内核安装，所以一个在沙箱内持有 root 的工作负载并不是自己那些限制之上的 root。在 VM 基质上，过滤器由一个*客户机内部*的 supervisor 进程安装，而它与工作负载共享一个内核（[overview.md](./overview.md) §12.2）。一个能附着到那个 supervisor、读它的内存或向它写入的工作负载，可以颠覆此后安装的每一条限制 —— 所以那些能伸进另一个进程地址空间的系统调用，在那里不是调试便利，它们就是绕过路径。

`syscall/1-vm` 是 `syscall/1` 再加上：

```yaml
deniedSyscalls:
  - ptrace              # 附着到另一个进程、检视并修改它
  - process_vm_readv    # 无需附着即可读取另一进程的内存
  - process_vm_writev   # 无需附着即可写入另一进程的内存
```

1. VM 基质上的部署**必须**默认采用 `syscall/1-vm` 而不是 `syscall/1`。两者都仍可由 `baselineVersion` pin 住，且一经发布都不可变（见下面的规则）。
2. 代价是真实的，且不加粉饰：在这个集合下，`strace`、`ltrace` 与 `gdb` 在沙箱内不工作，而 [exec.md](./exec.md) §3.4 仍然解析那些包装器 —— 一次调用会被 `exec` 放行，然后在系统调用边界上失败。需要沙箱内调试的部署显式 pin `syscall/1`，并接受 [overview.md](./overview.md) §12.2 对由此得到的保证所说的话。
3. 在 VM 基质上 pin `syscall/1` 的部署，**必须**在它所声明的每一个 `process` 与 `filesystem` 字段的 `knownLimitations` 里记录那条信任前提（[overview.md](./overview.md) §8.2.1 规则 5）。它**不得**在不加这项限定的情况下把它们声明为 `enforced`。
4. 容器基质**不得**默认采用 `syscall/1-vm`。在那里拒绝 `ptrace` 换不到这套论证所关心的任何东西，反而会白白弄坏调试器 —— 那里的强制执行无论如何都在工作负载之上。

版本化完全遵循 [filesystem.md](./filesystem.md) §6.2，理由也相同 —— 新的逃逸面会不断被发现，所以集合必须能够增长，而不打破那些已解析到旧版本的策略：

1. 基线集合是**带版本且不可变**的。`syscall/1` 是上面第一个集合、`syscall/1-vm` 是第二个；两者已发布的版本都**不得**被原地修改。
...
这里的 `tier: baseline` 并不是逐字节等于今天的行为 —— 基线系统调用集合是新的，而本模块此前并不存在。它是兼容性规则两处刻意例外之一（[overview.md](./overview.md) §9），并且受到与另一处相同的约束：集合是带版本、可 pin 的，且可通过 `baselineExceptions` 逐条逃出（§4.3）。
...
2. **restricted 分级的系统调用集合。** `tier: restricted` 目前展开为与 `tier: baseline` 相同的基线集合（[overview.md](./overview.md) §7.1）。`restricted` 是否应该引入第二个更大的带版本集合（追加 `mount`、`umount2`、`unshare`），而不是只收紧权限与持久化？这个问题的一项反对意见现在从一个意外的方向被回答了：`syscall/1-vm`（§4.3）确立了「可以有不止一个已发布集合共存、并由策略作者之外的东西来选定」，于是「第二个集合就是第二样要维护的东西」这项成本，本提案已经接受过一次了。剩下的仍是原来那个问题：一个分级究竟可不可以选定一个集合 —— 鉴于 §7.1 规则 1 把分级限制在模块字段的默认值上，而 `baselineVersion` 正是这样一个字段，答案大概是可以，真正的问题因而变成该收哪些条目。
...
- 本文档的每一条要求都可以用按进程、非特权的机制表达（在进程创建时安装的 seccomp 式过滤器、内核的 no-new-privileges 标记、bounding 能力集、按会话的进程组跟踪）。与本提案其他部分一样，规格固定的是*属性*，机制选型留待开放。影子评估（§6.1）也不例外：常用的过滤机制都提供一个"记录并允许"的动作，而那正是那里的规则 1 所要求的。
