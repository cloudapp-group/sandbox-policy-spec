...
| 被排除 | 原因 |
| --- | --- |
| `ptrace` | 调试器要用它，而 [exec.md](./exec.md) §3.4.2 把 `strace` 与 `ltrace` 列在平台会解析的包装器之中。在基线里拒绝它，会与提案已经预期能工作的一个面自相矛盾。**在 VM 基质上这条排除不成立** —— 见下面那个与基质相关的集合。 |
...
2. 代价是真实的，且不加粉饰：在这个集合下，`strace`、`ltrace` 与 `gdb` 在沙箱内不工作，而 [exec.md](./exec.md) §3.4.2 仍然解析那些包装器 —— 一次调用会被 `exec` 放行，然后在系统调用边界上失败。需要沙箱内调试的部署显式 pin `syscall/1`，并接受 [overview.md](./overview.md) §12.2 对由此得到的保证所说的话。
