...
- **本模块与基质无关，而这一点值得写明，因为本文档的一个早期版本暗示了相反的意思。** 上面每一种机制都是按*进程*的、不是按内核的：容器基质上的 `securityContext` 与 VM 基质上的客户机内配置抵达同样的属性，而任一基质上的部署都被期望把 §3 与 §4 声明为 `enforced`（[overview.md](./overview.md) §12.2）。在共享内核上收窄一个沙箱的系统调用面不可能波及邻居，因为过滤器是附着在进程上而不是附着在内核上的。独占内核买到的是在宿主级设置上的余地，而不是过滤的能力 —— 把两者混为一谈，正是一份规格如何获得一项它并不需要的基质要求。
