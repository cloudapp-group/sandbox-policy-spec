...
- 在 VM 基质上每个沙箱独占一个内核，因此 §4.3 的要求可由客户机内、无特权、按进程的机制满足（如带按进程规则集的 LSM，或等价的系统调用级强制）。本规格刻意只规定*属性*，不规定机制。[process.md](./process.md) §4.5 对系统调用面提出了同样的要求，且预期两者可由同一套机制满足。
- **本模块是两种基质真正存在差异的两处之一**（[overview.md](./overview.md) §12.2），而把这个差异的形状讲具体、而不是留一句"取决于平台"，是值得的：

  | 路径 | §4.2 如何落地 | 应声明的能力状态 |
  | --- | --- | --- |
  | VM 基质，客户机内带按进程规则集的 LSM | 直接落地：模式变成附着到每个进程上的规则集 | `enforced` |
