...
- 在 VM 基质上每个沙箱独占一个内核，因此 §4.3 的要求可由客户机内、无特权、按进程的机制满足（如带按进程规则集的 LSM，或等价的系统调用级强制）。本规格刻意只规定*属性*，不规定机制。[process.md](./process.md) §4.5 对系统调用面提出了同样的要求，且预期两者可由同一套机制满足。请注意，在那个基质上，该机制是从客户机*内部*安装的，这给结果、而不是给机制附加了一条信任前提（[overview.md](./overview.md) §12.2）：这些规则在工作负载无法触及那个安装了它们的进程期间成立。
- **本模块是两种基质真正存在差异的两处之一**（[overview.md](./overview.md) §12.2），而把这个差异的形状讲具体、而不是留一句"取决于平台"，是值得的：

  | 路径 | §4.2 如何落地 | 应声明的能力状态 |
  | --- | --- | --- |
  | VM 基质，客户机内带按进程规则集的 LSM | 直接落地：模式变成附着到每个进程上的规则集 | `enforced`，受 §12.2 那条前提限定 |
  | 容器基质，内核提供的非特权按进程路径规则集 | 在该接口所覆盖的操作范围内直接落地 | `enforced`；在该接口未覆盖本模块所规定的某个操作时为 `partial` |
  | 容器基质，由宿主管理、按沙箱生成的 LSM profile | 直接落地，但需要策略对象无法强制要求的节点级配合 | 在部署掌控自己节点的情况下为 `enforced` |
  | 两者都不可用 | — | `unsupported` |
