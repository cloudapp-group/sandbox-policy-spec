...
本规格定义 `SandboxPolicy` 对象的身份（`identity`）子策略：**行动的主体以什么名字被知晓，沙箱可以使用哪些凭据，以及它们以何种形式抵达沙箱**。它治理四件事：

- **身份命名** —— Agent 与工具以何种名字在平台内行动与被抵达（§2）。
...
一个沙箱里跑着一个 Agent，这个 Agent 调用工具。这两端就是身份的全部，它们共用一套命名格式：

| 对象 | 命名 |
| --- | --- |
| Agent | `agent://<tenant_type>/<cluster_id>/<cluster_namespace>/<agent_name>` |
| 工具 | `tool://<tenant_type>/<cluster_id>/<cluster_namespace>/<tool_name>` |

`tenant_type` 是一个封闭枚举，不是自由文本：
...
1. 五段合起来确定一个对象：`<scheme, tenant_type, cluster_id, cluster_namespace, name>`。两个名字**必须**逐段比较；同一个名字在不同的 `cluster_namespace` 下是另一个对象，没有任何隐含的继承或可见性。
2. 每一段**必须**非空，且**不得**包含 `/`。这是本规格对字符集的全部约束 —— 更严的约束属于平台自己的对象注册表。
3. 名字不是凭据，也不是秘密。知道一个 `tool://` URI —— 甚至把它写进策略 —— 不产生任何权限；URI 可以出现在审计与错误载荷中，不受 §9 脱敏规则的约束，因为它指名对象，不携带开门的钥匙。
4. URI 指名对象，不指名一次运行。同一个 Agent 在两个沙箱中运行是两个身份实例（§6.3），共用同一个名字 —— 实例的区分发生在签发，不发生在命名。
5. 这套命名在本模块内是承重的：`audience`（§6.2）以它指名一个身份被签往的对象；审计（§10）以它归因一次行动来自哪个身份；[network.md](./network.md) 的 `identity` 可达性模式用它解析对象，而不是解析地址。
...
policy:
  identity:
    ...
    workloadIdentity:
      ...
      audience:       [string]                   # enabled 时必填；§2 的 URI
      ttlSec:         int                        # 默认：900
...
| 模式 | 工作负载看到 | 凭据可用 |
| --- | --- | --- |
| `proxy` | 什么都看不到 | 可用，限于所声明的目的地与作用域（§5） |
...
2. 在 `file` 与 `env` 之下，那个值就在沙箱内部，而本模块的保证到此为止。一个工作负载读得到的凭据，就是一个工作负载能发到 `network` 所允许的任何地方、能打进 stdout、能写进一个日后被快照运出去的文件（[overview.md](./overview.md) §8.3.2）的凭据。这两种模式之所以存在，是因为真实的工作负载使用那些会去读 `~/.aws/credentials` 或 `AWS_ACCESS_KEY_ID` 的 SDK，而一份只提供 `proxy` 的规格会被绕开而不是被采纳。它们不等价于 `proxy`，而 §7 让平台把这一点说出来。
3. `none` 的存在，是为了让一条绑定可以在被打开之前先被声明、被评审、被纳入版本控制，也为了让吊销一条绑定成为一次单字段修改 —— 那让声明保持可见，而不是删掉它曾存在过的证据。
4. **暴露在合并时只能收窄**（§11）：排序是 `none` > `proxy` > `file` > `env`。一个选了 `proxy` 的低优先级来源，**不得**被更高优先级的来源放宽为 `file` 或 `env`。

### 4.1 `proxy` 对部署提出的要求，直说
...
4. `destinations` 不放宽 `network`。一个在此被许可、却被 `network` 拒绝的目的地仍然不可达；两个模块取交集。反过来也成立、且值得写明：`network.internal.mode: allow` 使云元数据端点可达，而从它取得的凭据是本模块从未签发、也无法吊销的（[network.md](./network.md) §2.2.3）。因此一份把该设置与一个 `proxy` 绑定配对的策略携带 `metadata_endpoint_reachable` 警告，因为 §4.1 的保证只对本模块所控制的凭据成立。一条其目的地在生效网络策略之下全部不可达的绑定，**必须**作为 `policyWarnings` 条目 `{field: "policy.identity.secrets", name, reason: "destinations_unreachable"}` 上报 —— 一个以为某个凭据正在被使用、而其实没有任何东西能抵达其目的地的作者，应该在创建时就知道这件事。
5. 当一个凭据是短期的、且由平台签发时（§6），该被签发凭据自身的 audience **必须**在签发系统支持的范围内被约束到该绑定的目的地上。在令牌上强制的作用域强于在代理上强制的作用域，因为它在代理出错时依然成立。

## 6. 工作负载身份与生命周期

1. 当 `workloadIdentity.enabled` 为 true 时，平台**必须**能够把该沙箱承载的 Agent 作为一个独立、可验证的身份呈现给外部服务，该身份以 §2 的 `agent://` 命名，且**必须**被限定到 `audience` —— `audience` 的每个条目是 §2 的 URI，指名这个身份被签往的对象。一个没有 audience 的身份，是一个多了几个步骤的 bearer token。
2. `enabled` 为 true 时 `audience` **必填**。以 `400 INVALID_POLICY` 拒绝一个空 audience 是刻意的：缺失 audience 的失败模式，是一个凭据被某个没人打算授权的服务接受。一个不符合 §2 格式的条目以同样方式被拒绝：解析不成对象的 audience 约束不了任何东西。
3. 该身份**必须**绑定到沙箱实例，而不是绑定到模板、策略档或调用方。同一个模板出来的两个沙箱是两个身份实例，使吊销其中之一不会吊销另一个，也使审计轨迹能把一次行动归因到某个身份与承载它的那个沙箱。
...
7. 在 `exposure: file` 或 `env` 之下，规则 5 与 6 **按构造只是尽力而为**，而平台**必须**把这一点说出来：一个已被读入工作负载内存的值无法被召回。这是那两种模式的具体代价，而它属于能力报告（§7），不属于一条脚注。
...
| `workloadIdentity.enabled` | `bool?` | — | `managed` 之下为 `true` | §6.1。 |
| `workloadIdentity.audience` | `[string]?` | `enabled` 为 true 时**必须**非空，且条目符合 §2。 | `[]` | §6.2。 |
| `workloadIdentity.ttlSec` | `int?` | > 0，≤ 部署最大值。 | `900` | §6.4。 |
| `defaultExposure` | `enum?` | `proxy` \| `file` \| `env` \| `none` | `proxy` | 省略了 `exposure` 的绑定所用的默认值。 |
| `secrets` | `[SecretBinding]?` | 按 §3.1。`name` 唯一。 | `[]` | 已声明的各凭据。 |
| `onViolation` | `enum?` | `deny` \| `kill` | `deny` | §8。 |
| `audit` | `enum?` | `none` \| `metadata` | `none` | **普通**凭据使用的审计级别（§10）。它不压制违规事件（[overview.md](./overview.md) §8.1.4）。 |
...
`baseline` 设 `defaultExposure: file` 而不是 `proxy`，理由值得写出来：`proxy` 需要一个出站强制点（§4.1），而不是每个部署都有；一个解析到某个 `unsupported` 字段的分级，会让 `tier: baseline` 在那些部署上于 `enforcement: strict` 之下直接失败。`file` 至少让暴露成为显式且可审计的。`restricted` 选择 `proxy`，而一个无法强制它的部署会在创建时发现 —— 那正是对的时刻。
...
| 动作 | 结果 |
| --- | --- |
| `deny`（默认） | 请求在不带凭据的情况下被转发（§5.2），或该操作以本模块的错误失败（§9）。进程继续运行。 |
| `kill` | 那个违规**进程**被终止。 |
...
3. 两种动作都会产生违规事件，且在任何审计级别下都产生（§10）。

## 9. 错误

| 错误码 | HTTP | 载荷 | 何时 |
| --- | --- | --- | --- |
| `INVALID_POLICY` | 400 | `{field, reason}` | `enabled: true` 下 `audience` 为空、或其条目不符合 §2；`proxy` 下 `destinations` 为空；该模式不使用的字段（§3.1.3）；`ttlSec` 超过部署最大值。 |
...
| `POLICY_IDENTITY_EXPOSURE_DENIED` | OS 级失败或 `403`；审计事件 | `{name, exposure, action}` | 工作负载试图读取一个其暴露模式将其隐藏的凭据（§4.1）。 |
| `POLICY_IDENTITY_DESTINATION_DENIED` | 仅审计事件 | `{name, destination}` | 因目的地不匹配而未附加凭据（§5.2）。该请求本身照常进行，因此这不是一个返回给调用方的错误。 |
| `POLICY_UNSUPPORTED` | 400 | `{field, state, capabilityVersion}` | 在 `enforcement: strict` 之下，策略使用了本部署声明为 `unsupported` 的暴露模式 —— 最常见的是 `proxy`（§4.1）。 |
| `POLICY_GRANT_INVALID` | 400 | `{field, reason}` | 授权指向一个不可授权字段（§11.1）。 |
...
2. `audit: metadata` 增加的是对**普通**凭据使用的记录：哪条绑定在何时被附加到了哪个目的地、以哪个身份（§2）。不包括凭据本身。
3. 平台所签发或附加的每一个凭据都**必须**可归因到一个沙箱、一个身份（§2）、一个绑定 `name` 与一个 `effectivePolicyVersion`。「14:03 时是哪个沙箱用了这个凭据？」**必须**能从审计流回答，因为那是一次事故的第一个问题。
4. 一条解析为 `exposure: file` 或 `env` 的绑定**必须**在沙箱创建时产生一条记录该残余暴露的审计事件（§6.7）。把一个可读的凭据交给部分可信的代码是一个事件，不是一个沉默的字段 —— 与 [filesystem.md](./filesystem.md) §9.4 给 `baselineExceptions` 的处理相同。
...
| `defaultExposure` | 最严格者胜出：`none` > `proxy` > `file` > `env`（§4.4）。 |
...
`destinations` 取交集这一条值得一个注解，因为它是本模块合并中唯一可能产出令人意外结果的地方：一条限定到 `*.example.com` 的模板绑定，与一条同名、限定到 `api.other.com` 的请求绑定，交集为**空**，于是该绑定不附加到任何目的地。那是正确的只能收窄结果，而它**必须**作为 `destinations_unreachable` 警告（§5.4）上报，而不是留给工作负载以一次认证失败的形式去发现。
...
3. **伪造请求头不会导致附加。** 一个连到 `destinations` 之外地址、却带着指名某个被允许目的地的 `Host` 头的连接，不会得到该凭据（§5.3）。
...
6. **audience 是强制的且被命名。** `workloadIdentity.enabled: true` 配空 `audience`、或配一个不符合 §2 格式的条目，都以 `400 INVALID_POLICY` 被拒。
7. **身份按实例。** 同一模板出来的两个沙箱呈现两个身份实例（§6.3），而吊销其中之一不影响另一个继续工作。
8. **命名空间不继承。** 两个 `cluster_namespace` 下的同名工具是两个对象：为其中一个声明的可达性或 `audience` 不为另一个打开任何东西。
9. **续期无需配合。** 一个在超过 `ttlSec` 的时间跨度内持续发起已认证请求的沙箱，在自己不执行任何刷新的情况下持续成功（§6.5）。
10. **吊销是实时的。** 移除一条绑定会让后续请求不再被认证，且无需重启沙箱（§6.6）。
11. **残余暴露是诚实的。** 在 `exposure: env` 之下，吊销不会撤回工作负载已读到的值，生效策略上报那处残余暴露，且创建时已产生 §10.4 的事件。
12. **载荷不泄露。** 本模块产出的任何错误、警告或审计事件都不包含凭据值或 `secretRef`，包括在「秘密无法解析」与「暴露被拒」这两种情形下（§9）。
13. **合并只能收窄。** 一条 `exposure: proxy` 的模板绑定，不能被一条同名请求绑定改成 `env`；`destinations` 取交集；`ttlSec` 取最小值；`audience` 取交集。任何试图放宽其中之一的请求按 [overview.md](./overview.md) §5 处理 —— 被拒绝或被上报，绝不静默生效。
14. **授权。** 一份追加了 `proxy` 绑定的授权在沙箱不做任何动作的情况下到期，之后请求不再被认证。试图改变 `exposure`、抬高 `ttlSec` 或设 `mode: unrestricted` 的授权被拒绝。
15. **不支持的 proxy 以 fail-closed 处理。** 在一个把 `exposure: proxy` 声明为 `unsupported` 的部署上，请求它的策略在 `enforcement: strict` 之下以 `400 POLICY_UNSUPPORTED` 被拒，而在 `bestEffort` 之下被接受、该绑定被上报为失效 —— 两种情况下它都不会被静默降级为 `file` 或 `env`。
16. **分级默认值。** `tier: restricted` 且不带任何身份字段，解析为 `mode: managed`、`defaultExposure: proxy`、`workloadIdentity.enabled: true`，且没有任何秘密 —— 一个不以任何身份认证、也不携带任何东西的沙箱。`tier: compatibility` 解析为 `mode: unrestricted`，且不改变今天行为的任何部分。
17. **影子评估。** 在 `tier: baseline` 配 `auditTier: restricted` 下，一条处于 `file` 模式的绑定继续工作，并产生一条指名该绑定与 `proxy` 的 `shadow: true` 发现。不为该影子评估签发任何凭据，且沙箱内部可观察到的一切没有差别。
18. **生命周期。** 一个从快照恢复的沙箱不继承秘密绑定（[overview.md](./overview.md) §8.3）；已认证请求会失败，直到那些绑定被重新建立。

## 13. 开放问题

1. **凭据类型的分类。** 本规格把每个凭据都当作一个被附加到出站请求上的不透明值。真实的凭据各不相同：bearer token 被附加到一个请求头上，AWS 签名是在请求之上计算出来的，客户端证书用在握手中，而数据库口令走的是一个本模块无法解析的协议。`SecretBinding` 是否需要一个带按类型附加语义的 `type`，还是说 `proxy` 模式诚实地只限于 HTTP 形状的凭据 —— 若是后者，那条限制**必须**写进 §4 而不是暗示出来？
2. **非 HTTP 目的地。** §5 是以目的地、方法与请求路径来写的，而那预设了 HTTP。一个用于 Postgres 连接的 `proxy` 模式凭据没有方法也没有路径，而代理将不得不去说那个线协议。`proxy` 在 v1 是否限定于 HTTP/HTTPS，而非 HTTP 凭据必然是 `file` 或 `env`？
...
7. **命名的权限与生命周期。** 谁可以在一个命名空间里注册 `agent://` 与 `tool://` 名字，一个对象被删除后它的名字多久可以被复用？回收一个名字会让旧的审计轨迹指向一个不相关的继任者 —— 回答「14:03 时这个身份做了什么」之前，得先确认它还是不是当时那一个。
...
- **为什么沙箱不在这套命名里。** 沙箱是一个执行环境，不是以 URI 行动或被抵达的对象 —— 行动的是它承载的 Agent，被抵达的是工具。平台已经在按沙箱 ID 标识实例（§10 的审计事件），再给沙箱一个身份 URI，就是用第二套标识回答同一个问题，并让「环境」与「主体」两个概念长得相像。§2 因此只命名两类对象；§6.3 把身份绑定到沙箱实例 —— 名字与实例，各司其职。
...
- **关于偏好 `proxy` 但不强制它。** 本模块的一个早期草案把 `proxy` 作为唯一模式。它因 §4.2 给出的理由被否决：SDK 生态从文件与环境变量读取凭据，而一门无法表达部署实际所做之事的策略语言会被绕开而不是被采纳。折中之处在于 `file` 与 `env` 仍然可用、仍然在创建时被审计，并且被如实描述为它们本来的样子 —— §6.7 与 §10.4 中关于残余暴露的措辞是刻意直白的。
