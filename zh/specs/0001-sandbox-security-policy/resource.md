...
2. **显式上报（兜底）。** SDK 提供 `sandbox.report_usage()`，供调用方上报响应侧计量观测不到的用量（非 HTTP 供应商、流式边界场景）。上报量计入计数器并带独立来源标记。
...
3. `state.exhausted` 列出当前已超限的（维度, 窗口）对；`state.held` 在 hold 待决期间为 true。
4. SDK 以 `sandbox.resource` 暴露同一对象。
