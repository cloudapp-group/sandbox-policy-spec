# Sandbox Security Policy 规范

[![License](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](LICENSE)
[English](README.md) | [中文](README.zh-CN.md)

本仓库包含 **提案 0001：Sandbox Security Policy** —— 一个面向沙箱执行环境的声明式、可复用、默认安全的策略框架。

沙箱不仅仅是网络端点。它运行的是部分可信、由 Agent 生成的代码，因此其能力边界必须在一个统一的对象中覆盖 **网络、文件系统、执行、进程行为、身份和资源** 消耗。

![一个沙箱的能力边界：一条经过认证的通往工具的路径、三面被强制的墙，以及自上方计量供给的预算](assets/sandbox-policy-boundary.png)

*Agent 只能穿过一次经过认证的身份才能抵达工具；三面被强制的墙约束什么可以抵达它、它可以抵达什么、以及它在脚下那个运行时上可以做什么；而它的预算从上方被计量供给。*

---

## 这是什么？

本提案定义了 `SandboxPolicy` 对象：它相当于云安全组，但扩展到了沙箱的完整执行面。

| 模块 | 覆盖范围 |
| --- | --- |
| **Network** | L3/L4/L7 出站允许/拒绝、入站门控、DNS 学习、公共流量控制 |
| **Filesystem** | 宿主机挂载边界与沙箱内路径访问策略（`denyPaths`、`readOnlyPaths`、`writableRoots`） |
| **Exec** | 命令白名单/黑名单、用户限制、超时上限、并发限制、审计 |
| **Process** | 已运行进程的提权、持久化与系统调用策略 |
| **Identity** | 工作负载身份、秘密暴露模式、目的地绑定的凭据注入、TTL 与吊销 |
| **Resource** | 请求/磁盘速率上限与整形计数、窗口化 Token 预算（分钟–月 + 生命周期）、Token 计量、超限处置 |

规范以七文档集合的形式组织在 `specs/0001-sandbox-security-policy/` 目录下，JSON Schema 位于 `schema/`，合规性 fixtures 位于 `fixtures/`。

---

## 动机

每一台云 ECS 实例都附带一个安全组：一种声明式、可复用的定义，说明*这台机器可以与谁通信*。它之所以有效，是因为具备以下特性：

- **声明式** —— 用户表达意图，而非机制。
- **默认启用** —— 每个实例默认都有一份合理的基线策略。
- **可复用** —— 一个安全组可绑定多个实例，变更会传播到所有实例。
- **统一性** —— 一套语法、一种评估顺序、一条审计链路。

沙箱恰恰需要这些特性，并且要将它们扩展到执行域。与 ECS 实例不同，沙箱运行的是**由 Agent 生成的、部分可信的代码**。其爆炸半径更广：数据外泄、凭据窃取、宿主机挂载滥用、提权、失控循环以及 LLM Token 浪费。

| ECS 实例 | 沙箱 |
| --- | --- |
| 运行人工编写、可信的工作负载 | 运行 Agent 生成、部分可信的代码 |
| 边界 = 网络可达性 | 边界 = 网络 **+ 文件系统 + 执行 + 进程 + 身份 + 资源** |
| 爆炸半径：数据外泄 | 爆炸半径：外泄 **+ 凭据窃取、宿主机挂载滥用、提权、失控循环、Token 浪费** |

### 当前的缺口

六个能力域目前的成熟度差异很大：

| 模块 | 当前已有能力 | 缺口 |
| --- | --- | --- |
| **Network** | 出站允许/拒绝、域名白名单、L7 规则、入站门控 | 字段分散；没有可复用的策略对象；没有统一的合并语义 |
| **Filesystem** | 宿主机挂载前缀白名单与每个挂载的 `readOnly` | 缺少对沙箱内敏感路径的保护；缺少路径级只读/拒绝策略 |
| **Exec** | 每次请求单独设置 `timeout`、`user`、`cwd` | 缺少沙箱级命令策略、用户限制、并发上限和审计 |
| **Process** | 无任何面向用户的能力 | 提权、持久化与系统调用暴露面完全没有策略表达 |
| **Identity** | 无 | 凭据以环境变量或文件抵达，沙箱内任何代码都能读到；没有工作负载身份、暴露模式、TTL 或吊销 |
| **Resource** | CPU/内存稳态配额；空闲超时 | 缺少请求与磁盘 I/O 的速率上限、窗口化 Token 预算、Token 计量和超限处置 |

如果没有单一策略对象，每个模块都会长出各自的配置风格、合并规则、默认值和审计格式。用户必须同时理解六个半成品系统；模板作者无法在一个地方表达"该模板的沙箱已被锁定"；未来的新模块还会引入第七、第八种方言。

### 为什么需要统一策略？

统一的 `SandboxPolicy` 为平台带来：

- **一种心智模型** —— *"沙箱只能做其策略允许的事情。"*
- **一套合并逻辑** —— 模板默认 + 策略画像 + 请求覆盖，所有模块共享。
- **一条默认安全基线** —— 默认安全，显式选择退出。
- **一处审计与观测入口** —— 每次拒绝都有结构化解释。

设计遵循六项原则：

1. **声明式** —— 表达意图，而非机制。
2. **缺省 ≠ 无限制** —— 缺失字段解析为文档化的安全默认值。
3. **默认安全，显式退出** —— 基线保护默认开启，除非主动关闭。
4. **拒绝可解释** —— 每次拒绝都携带命中的规则或耗尽的维度。
5. **强制 enforcement** —— 每条规则都在工作负载无法绕过的点强制执行。
6. **下游单一表示** —— 遗留字段、内联策略和策略画像都归一为同一个有效对象。

---

## 仓库结构

```
.
├── schema/0001/             # SandboxPolicy 的 JSON Schema
├── fixtures/                # 与适配器无关的合规性 fixtures
└── specs/0001-sandbox-security-policy/
    ├── en/
    │   ├── overview.md      # Shared model, merge semantics, principles, tiers, shadow evaluation, grants, compatibility
    │   ├── network.md       # Network sub-policy
    │   ├── filesystem.md    # Filesystem sub-policy
    │   ├── exec.md          # Command execution sub-policy
    │   ├── process.md       # Privilege, persistence, and system-call sub-policy
    │   ├── identity.md      # Workload identity, secret exposure, credential scope
    │   └── resource.md      # Rate ceilings, token budgets, and governance
    └── zh/
        ├── overview.md      # 共享模型、合并语义、原则、分级、影子评估、限时授权、兼容性
        ├── network.md       # 网络子策略
        ├── filesystem.md    # 文件系统子策略
        ├── exec.md          # 命令执行子策略
        ├── process.md       # 提权、持久化与系统调用子策略
        ├── identity.md      # 工作负载身份、秘密暴露、凭据作用域
        └── resource.md      # 速率上限、Token 预算与治理
```

---

## 语言与事实源

`specs/0001-sandbox-security-policy/en/` 下的英文文档是**规范文本**。`zh/` 一套是译文，以尽力而为的方式维护，因而可能滞后。两者不一致时，以英文为准。

这是一个刻意的选择：一份规范性文档若有两个各自权威的副本，必然漂移；而一份已经漂移的规范性文档，比一份没有译文的规范性文档更糟。要实现本规范的读者应以 `en/` 为依据；`zh/` 的存在是为了降低阅读与评审它的成本。

---

## 参与贡献

本提案目前处于 **Draft（草案）** 状态，正在接受社区讨论。欢迎贡献：

1. 在跟踪 issue 中发起讨论或评论。
2. 通过 Pull Request 提交修改；请每个 PR 保持一个逻辑变更。
3. 规范性修改**必须**落在 `en/`。在同一个 PR 中一并更新 `zh/` 是首选；如果做不到，请在 PR 描述中说明，让缺口是可见的而不是无声的。
4. 只改 `zh/` 的 PR 属于译文修正，**不得**改变含义。若它改变了含义，那它应当先去 `en/`。

当你认领某个章节时，请在 `overview.md` 的 **Authors** 字段中添加你的名字。

---

## 许可证

本项目采用 [Apache License, Version 2.0](LICENSE) 授权。
